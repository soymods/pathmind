...
final class NodeInventoryCommandExecutor {
    ...
    // The Click Slot modes send exactly what the vanilla screen sends for the same input: left
    // and right click are PICKUP with buttons 0 and 1, shift-click is QUICK_MOVE, and F is SWAP
    // on button 40, which is the offhand's button index in AbstractContainerScreen. A node saved
    // before the modes existed loads with the default CLICK_SLOT_LEFT, and a null mode is the
    // same plain click.
    ...
    static int clickSlotButton(NodeMode mode) {
        if (mode == NodeMode.CLICK_SLOT_RIGHT) {
            return 1;
        }
