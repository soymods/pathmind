...
1. Add a complete, ordered row to the compatibility manifest and `supported_versions`. Add it to `fast_verification_versions` only if it represents a new fast-CI boundary.
...
Pull requests and non-default branch pushes use the manifest's representative fast tier. Default-branch pushes, scheduled runs, and manual workflow dispatches build the full matrix. Every matrix job invokes `verifySelectedCompatibilityArtifacts`; publishing then confirms the complete expected jar set before replacing the rolling prerelease.
