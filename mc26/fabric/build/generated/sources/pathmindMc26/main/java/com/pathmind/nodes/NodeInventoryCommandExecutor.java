...
final class NodeInventoryCommandExecutor {
    ...
    private void performInventoryTransfer(MultiPlayerGameMode interactionManager, AbstractContainerMenu handler,
        ...
        if (moveEntireStack) {
            interactionManager.handleContainerInput(handler.containerId, sourceSlot, 0, ContainerInput.PICKUP, player);
            interactionManager.handleContainerInput(handler.containerId, targetSlot, 0, ContainerInput.PICKUP, player);
            interactionManager.handleContainerInput(handler.containerId, sourceSlot, 0, ContainerInput.PICKUP, player);
            return;
        }
        ...
        while (moved < moveCount) {
            ...
            interactionManager.handleContainerInput(handler.containerId, targetSlot, 1, ContainerInput.PICKUP, player);
    ...
    private void dropSlotCount(net.minecraft.client.Minecraft client,
        ...
        if (dropEntireStack || count >= available) {
            interactionManager.handleContainerInput(handler.containerId, handlerSlotIndex, 1, ContainerInput.THROW, client.player);
            finishInventoryClickUpdates(client, inventory);
            return;
        }

        if (count == 1) {
            interactionManager.handleContainerInput(handler.containerId, handlerSlotIndex, 0, ContainerInput.THROW, client.player);
            finishInventoryClickUpdates(client, inventory);
            return;
        }
    ...
    private boolean dropPartialStackAsSingleCursorClick(net.minecraft.client.Minecraft client,
        ...
        interactionManager.handleContainerInput(handler.containerId, sourceSlot, 1, ContainerInput.PICKUP, client.player);
        if (handler.getCarried() != null
            && !handler.getCarried().isEmpty()
            && handler.getCarried().getCount() == requestedCount) {
            interactionManager.handleContainerInput(handler.containerId, -999, 0, ContainerInput.PICKUP, client.player);
            finishInventoryClickUpdates(client, inventory);
            return true;
        }

        interactionManager.handleContainerInput(handler.containerId, scratchSlot, 0, ContainerInput.PICKUP, client.player);
        interactionManager.handleContainerInput(handler.containerId, scratchSlot, 1, ContainerInput.PICKUP, client.player);
        while (handler.getCarried() != null
            && !handler.getCarried().isEmpty()
            && handler.getCarried().getCount() > requestedCount) {
            interactionManager.handleContainerInput(handler.containerId, sourceSlot, 0, ContainerInput.PICKUP, client.player);
            interactionManager.handleContainerInput(handler.containerId, scratchSlot, 1, ContainerInput.PICKUP, client.player);
        }
    ...
    private void restoreScratchToSource(MultiPlayerGameMode interactionManager,
        ...
        if (scratch != null && scratch.hasItem()) {
            interactionManager.handleContainerInput(handler.containerId, scratchSlot, 0, ContainerInput.PICKUP, player);
            interactionManager.handleContainerInput(handler.containerId, sourceSlot, 0, ContainerInput.PICKUP, player);
        }
    }
