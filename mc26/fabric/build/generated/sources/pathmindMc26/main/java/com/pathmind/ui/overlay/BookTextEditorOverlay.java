...
public class BookTextEditorOverlay {
    ...
    private void renderWrappedText(GuiGraphicsExtractor context, Font textRenderer, String text,
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (c == '\n') {
                ...
                if (currentY >= y - lineHeight && currentY < y + areaHeight + lineHeight) {
                    context.text(textRenderer, Component.literal(currentLine.toString()), currentX, currentY,
                        applyPopupAlpha(UITheme.TEXT_PRIMARY, popupAlpha));
                }
    ...
    private void renderButton(GuiGraphicsExtractor context, Font textRenderer, Button button, int mouseX, int mouseY, float popupAlpha) {
