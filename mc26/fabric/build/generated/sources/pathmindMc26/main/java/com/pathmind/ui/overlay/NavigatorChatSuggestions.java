...
public final class NavigatorChatSuggestions {
    ...
    public void render(ChatScreen chatScreen, GuiGraphicsExtractor context, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < suggestions.size(); i++) {
            ...
            context.text(textRenderer, entry.command(), commandX, rowY, UITheme.TEXT_HEADER);
            context.text(textRenderer, entry.hint(), hintX, rowY, UITheme.TEXT_SECONDARY);
        }
    }

    public void tick(Minecraft client) {
        if (client == null || !(client.gui.screen() instanceof ChatScreen chatScreen)) {
            return;
        }
        EditBox input = resolveChatField(chatScreen);
    ...
    private List<SuggestionEntry> movementSuggestions(String root, String[] parts, boolean endsWithSpace) {
        ...
        if (partCount == 2 && endsWithSpace) {
            if ("block".equals(parts[1])) {
                ...
                suggestions.add(new SuggestionEntry("diamond", "nearest dropped item by id", "!" + root + " item diamond"));
                suggestions.add(new SuggestionEntry("cobblestone", "nearest dropped item by id", "!" + root + " item cobblestone"));
            }
        }
        return suggestions;
    }
    ...
    private void drawPanelBorder(GuiGraphicsExtractor context, int x, int y, int width, int height, int color) {
