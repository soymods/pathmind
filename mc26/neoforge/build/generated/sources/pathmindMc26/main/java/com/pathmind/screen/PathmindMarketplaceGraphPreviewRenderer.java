...
final class PathmindMarketplaceGraphPreviewRenderer {
    ...
    private void drawGraphPreview(GuiGraphicsExtractor context, int x, int y, int width, int height, boolean popup) {
        ...
        for (int lineX = x + 12; lineX < x + width - 8; lineX += 18) {
            context.verticalLine(lineX, y + 6, y + height - 7, gridColor);
        }
        for (int lineY = y + 10; lineY < y + height - 8; lineY += 14) {
            context.horizontalLine(x + 6, x + width - 7, lineY, gridColor);
        }
    }

    void renderSurface(GuiGraphicsExtractor context, int x, int y, int width, int height,
    ...
    private void drawPreviewConnection(GuiGraphicsExtractor context, int startX, int startY, int endX, int endY, int color, boolean popup) {
    ...
    private void renderPreviewNode(GuiGraphicsExtractor context, Node node, float offsetX, float offsetY, float scale, boolean interactive, boolean popup) {
        if (node == null) {
            return;
        }
        ...
        if (scale > 0.12f) {
            ...
            context.text(screen.textRenderer(), Component.literal(label), nodeX + 4, nodeY + 3,
    ...
    private void renderPreviewStickyNote(GuiGraphicsExtractor context, Node node, float offsetX, float offsetY, float scale,
        ...
        for (String line : wrapStickyNotePreviewText(screen.fallback(node.getStickyNoteText(), ""), bodyWidth)) {
            ...
            context.text(screen.textRenderer(), Component.literal(line), bodyX, textY, resolvedText, false);
            textY += lineHeight;
        }
    }
    ...
    private void renderNodeSockets(GuiGraphicsExtractor context, Node node, float offsetX, float offsetY, float scale, boolean popup) {
    ...
    void drawMinimalPreviewButton(GuiGraphicsExtractor context, int x, int y, int width, int height, boolean hovered) {
    ...
    void drawPreviewMinusIcon(GuiGraphicsExtractor context, int x, int y, int color) {
        context.fill(x + 4, y + 6, x + 10, y + 8, color);
    }

    void drawPreviewPlusIcon(GuiGraphicsExtractor context, int x, int y, int color) {
        context.fill(x + 4, y + 6, x + 10, y + 8, color);
