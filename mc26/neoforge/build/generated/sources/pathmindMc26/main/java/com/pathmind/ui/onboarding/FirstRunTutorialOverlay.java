...
public final class FirstRunTutorialOverlay {
    ...
    public void render(GuiGraphicsExtractor context, Font textRenderer, int mouseX, int mouseY,
        ...
        context.text(textRenderer, Component.literal(title), panelX + PANEL_PADDING, panelY + PANEL_PADDING, titleColor);
        context.text(textRenderer, Component.literal(count),
        ...
        for (String line : bodyLines) {
            context.text(textRenderer, Component.literal(line), panelX + PANEL_PADDING, textY, bodyColor);
    ...
    private void renderScrim(GuiGraphicsExtractor context, int screenWidth, int screenHeight, int[] spotlight, float alpha, boolean hasSpotlight) {
    ...
    private void renderSpotlight(GuiGraphicsExtractor context, int[] spotlight, int accentColor, float alpha) {
    ...
    private void renderConnector(GuiGraphicsExtractor context, int x, int y, int width, int height, int[] spotlight, int accentColor, float alpha) {
        ...
        if (Math.abs(sourceX - targetCenterX) > Math.abs(sourceY - targetCenterY)) {
            ...
            context.horizontalLine(sourceEdgeX, midX, sourceY, color);
            context.verticalLine(midX, Math.min(sourceY, targetY), Math.max(sourceY, targetY), color);
            context.horizontalLine(Math.min(midX, targetX), Math.max(midX, targetX), targetY, color);
        } else {
            int sourceEdgeY = sourceY < targetCenterY ? y + height : y;
            int midY = (sourceEdgeY + targetY) / 2;
            context.verticalLine(sourceX, Math.min(sourceEdgeY, midY), Math.max(sourceEdgeY, midY), color);
            context.horizontalLine(Math.min(sourceX, targetX), Math.max(sourceX, targetX), midY, color);
            context.verticalLine(targetX, Math.min(midY, targetY), Math.max(midY, targetY), color);
        }
    }
    ...
    private void renderProgressDots(GuiGraphicsExtractor context, int x, int y, int accentColor, float alpha) {
    ...
    private void renderButtons(GuiGraphicsExtractor context, Font textRenderer, int mouseX, int mouseY, int accentColor, float alpha) {
    ...
    private void renderButton(GuiGraphicsExtractor context, Font textRenderer, int x, int y, int width, int height,
        ...
        context.text(textRenderer, Component.literal(trimmed), textX, textY, AnimationHelper.withAlpha(palette.textColor(), alpha));
    }
