...
public final class ScrollbarHelper {
    ...
    public static void renderSettingsStyle(GuiGraphicsExtractor context, Metrics metrics, int trackColor, int borderColor, int thumbColor) {
        if (metrics.maxScroll() <= 0) {
            return;
        }
    ...
    public static void renderCutoffDividers(GuiGraphicsExtractor context, int left, int right, int top, int bottom,
                                            int scrollOffset, int maxScroll, int color) {
        if (maxScroll <= 0) {
            return;
        }
        if (scrollOffset > 0) {
            context.horizontalLine(left, right, top, color);
        }
        if (scrollOffset < maxScroll) {
            context.horizontalLine(left, right, bottom - 1, color);
        }
    }
